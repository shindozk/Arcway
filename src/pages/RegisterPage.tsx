...
export default function RegisterPage({ onRegister, onSwitchToLogin, onSkip, loading, error, clearError }: RegisterPageProps) {
  ...
  const validate = (): boolean => {
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setValidationError(t('register.invalidEmail')); return false; }
    if (password.length < 6) { setValidationError(t('register.shortPassword')); return false; }
    if (password !== confirmPassword) { setValidationError(t('register.passwordMismatch')); return false; }
    setValidationError('');
    return true;
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    playSound('tap');
    try { await onRegister(email, password, username); playSound('success'); } catch { playSound('error'); }
  };
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', width: '100%', background: 'var(--md-sys-color-surface)', padding: '24px' }}>
      <div style={{
        ...
        borderRadius: '24px', padding: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px',
        ...
        <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <input type="text" placeholder="Username" value={username}
            onChange={(e) => { setUsername(e.target.value); clearError(); setValidationError(''); }}
            style={{ height: '48px', padding: '0 16px', borderRadius: '12px', border: '1px solid var(--md-sys-color-outline)', backgroundColor: 'var(--md-sys-color-surface-container-lowest)', color: 'var(--md-sys-color-on-surface)', fontSize: '15px', outline: 'none', width: '100%', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
            onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--md-sys-color-primary)'; }}
            onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--md-sys-color-outline)'; }}
          />
          <div style={{ position: 'relative' }}>
            <input type={showPassword ? 'text' : 'password'} placeholder={t('register.password')} value={password}
          ...
          <input type={showPassword ? 'text' : 'password'} placeholder={t('register.confirm')} value={confirmPassword}
            onChange={(e) => { setConfirmPassword(e.target.value); clearError(); setValidationError(''); }}
            style={{ height: '48px', padding: '0 16px', borderRadius: '12px', border: '1px solid var(--md-sys-color-outline)', backgroundColor: 'var(--md-sys-color-surface-container-lowest)', color: 'var(--md-sys-color-on-surface)', fontSize: '15px', outline: 'none', width: '100%', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
            onFocus={(e) => { e.currentTarget.style.borderColor = 'var(--md-sys-color-primary)'; }}
            onBlur={(e) => { e.currentTarget.style.borderColor = 'var(--md-sys-color-outline)'; }}
          />
          <button type="submit" disabled={loading}
            style={{ height: '48px', borderRadius: '12px', border: 'none', backgroundColor: 'var(--md-sys-color-primary)', color: 'var(--md-sys-color-on-primary)', fontSize: '15px', fontWeight: 500, cursor: 'pointer', opacity: loading ? 0.6 : 1, transition: 'opacity 0.2s' }}>
        ...
        <p style={{ fontSize: '13px', color: 'var(--md-sys-color-on-surface-variant)', margin: 0 }}>
          ...
          <button onClick={() => { playSound('tap'); onSwitchToLogin(); }} style={{ background: 'none', border: 'none', color: 'var(--md-sys-color-primary)', cursor: 'pointer', fontSize: '13px', fontWeight: 500, fontFamily: 'inherit' }}>
            {t('register.signin')}
          </button>
        </p>
      </div>
